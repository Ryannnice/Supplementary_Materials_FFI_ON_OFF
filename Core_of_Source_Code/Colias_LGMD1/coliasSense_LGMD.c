...
uint8_t Calc_LGMDs_OutOfChannel(LGMD_pControlTypedef* hLGMD)
{
	...
	int16_t temg;
	float a2,*Won_base,*Woff_base,sum_ffi,ffi;
  ...
  for(n=0;n<ncell;n++)
	{
		if(*diff_image>=0)
		{
			...
			*cur_Poff = (uint8_t)(0+a1*(*pre_Poff));
			//*cur_Pon = *diff_image+a1*(*pre_Pon);
			//*cur_Poff = 0+a1*(*pre_Poff);
			//*cur_Pon = *diff_image;
			//*cur_Poff = 0;
		}
		else
		{
			...
			*cur_Poff = (uint8_t)(abs(*diff_image)+a1*(*pre_Poff));
			//*cur_Pon = 0+a1*(*pre_Pon);
			//*cur_Poff = abs(*diff_image)+a1*(*pre_Poff);
			//*cur_Pon = 0;
	...
	for(i=1;i<height-1;i++)
	{
		...
		for(j=1;j<width-1;j++)
		{
			...
			center = *(cur_Pon+j);
			near = (*(cur_Pon+j-width)>>2)+(*(cur_Pon+j-1)>>2)+(*(cur_Pon+j+1)>>2)+(*(cur_Pon+j+width)>>2);
			diag = (*(cur_Pon+j-width-1)>>3)+(*(cur_Pon+j-width+1)>>3)+(*(cur_Pon+j+width-1)>>3)+(*(cur_Pon+j+width+1)>>3);
			pre_center = *(pre_Pon+j);
			pre_near = (*(pre_Pon+j-width)>>2)+(*(pre_Pon+j-1)>>2)+(*(pre_Pon+j+1)>>2)+(*(pre_Pon+j+width)>>2);
			pre_diag = (*(pre_Pon+j-width-1)>>3)+(*(pre_Pon+j-width+1)>>3)+(*(pre_Pon+j+width-1)>>3)+(*(pre_Pon+j+width+1)>>3);

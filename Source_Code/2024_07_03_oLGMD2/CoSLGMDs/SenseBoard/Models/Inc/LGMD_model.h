...
typedef struct
{
	...
	uint8_t S_ON[Layer_Height][Layer_Width];
	uint8_t S_OFF[Layer_Height][Layer_Width];
...
typedef struct
{
	...
	//uint8_t On_Tpm;         //the threshold of PM pathway in ON channel
  //uint8_t Off_Tpm;        //the threshold of PM pathway in OFF channel	
	...
	uint8_t Clip;
  float On_Tpm;         //the threshold of PM pathway in ON channel
  float Off_Tpm;        //the threshold of PM pathway in OFF channel		
...
typedef struct
{
	...
	float FFI_out[2];	//FFI output in ON channel
	float FFI_out1[2]; //FFI output in OFF channel

...
char timed_data_transmit(void)
{
	...
	if (counter>=UT_Dtimer.period)//excecute
	{
		...
		if (UT_Dtimer.tasks_remain)
		{
			...
			//printf("%f %f\n",hCoS->hHBIO->Instance->timerlog[1]-hCoS->hHBIO->Instance->timerlog[0]);
			
			//printf("%d \r\n",hCoS->hHBIO->Instance->timerlog[1]-hCoS->hHBIO->Instance->timerlog[0]);
			
			printf("%d %.2f %d \r\n\n",(101-UT_Dtimer.tasks_remain), LGMD.Results.LGMD_output[hLGMD.currentDiffImage],hCoS->hHBIO->Instance->timerlog[1]-hCoS->hHBIO->Instance->timerlog[0] ) ;

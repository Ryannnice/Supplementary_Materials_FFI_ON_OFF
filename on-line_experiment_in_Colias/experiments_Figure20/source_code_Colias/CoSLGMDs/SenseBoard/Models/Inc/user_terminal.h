...
typedef struct
{
	...
	float maxnum;
	u8 ctrl;
}UT_FvalRangeTypedef; // F val

typedef struct
{
	B32TypeIDdef type;
	const char* name; // cannot change centent
	const char* helpwd;
...
typedef struct 
{
	...
	B32TypeIDdef type;
	char* name;
	char* helpwd; //these three above is included in "hDetails" in the new_version
...
typedef struct
{
	...
	char ErrID;
	char bzero[2];
	char* Cargv[UT_MaxArgs]; // #define UT_MaxArgs 16
}CMDargTypedef;


typedef struct
{
	// __IO : 告诉编译器，每次对这个变量的访问都应该从内存中读取，而不要使用缓存
